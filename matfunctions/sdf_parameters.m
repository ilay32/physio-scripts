function [coeffs,cp,long_term_end] = sdf_parameters(timeinterval,sdf,good,transition,long_term_end)
% SDF_PARAMETERS.M	Calculates Stabilogram diffusion paramteters based upon input of SDF function
% Usage: [coeffs,cp,long_term_end] = sdf_parameters(timeinterval,sdf,good,transition,long_term_end)
% Inputs:
% 	TIMEINTERVAL = column vector of time intervals
% 	SDF = [sdfx, sdfy] (100Hz data is assumed)
...
long_term_end = [long_term_end long_term_end long_term_end];
...
sdf(:,3) = sdf(:,1)+sdf(:,2);
   ...
   % Calculate derivative of log-log plots- 100Hz data assumed
   % LPFilter the sdf at 20Hz to remove HF system noise for CP determination- zero phase
   	[filt_sdf, begin]=lpfilt(sdf,20,100,100,'y','cheby',4); % Filter transients removed
...
for column = 1:3
   ...
   % Initialize counter & future mean
      row =1; cont = 2; % Due to filter transient removal, row 1 = .05s, or 20Hz 
      ...
      %while ((B(row,column)) >1) | ( (cont) >1)
      %cont = mean(B(row:row+10,column)) % 10 samples selected based upon tradeoff of increased sensitivity
      	row = row + 1;
   	end;
  ...
  %subplot(2,1,2), semilogx(filt_time,B(:,1:3))
   ...
   if any(short_term_end == 1)
      error('Error: Trial exhibits antipersistence at start!')
   end;
   ...
   % Set beginning of short term to .05- don't use info above 20Hz (100Hz data assumed)
   %short_term_regress_begin = 5;
   
   
   %disp(['Short term ' num2str(short_term_regress_begin/100) ' to ' num2str(short_term_regress_end/100) 's, Long Term ' num2str(long_term_regress_begin/100) ' to ' num2str(long_term_end/100) 's.']);
   
   % Check for error
   for i = 1:3
      if long_term_end(i) <=long_term_regress_begin(i)-10
         ...
         disp(['Error in identification of critical time- dimension ' num2str(i) '. Extending to ' num2str(long_term_end(i)/100) 's.']);
      end;
   end;
   ...
   if good ~= 0 % Check long term region for good fit of regression
      if myDxl_stats(1,1) < good		% .7225 Corresponds to r=.85
      	a= sin(100*(.01:.01:300));
	%      sound(a);
      	disp(['Check end of long term region- r2 is ' num2str(myDxl_stats(1,1))]);
      	figure
      		plot(sdf(:,1))
      	...
      	long_term_end = input(['2000 does not work for X. Long term region goes from ' num2str(short_term_end(1)) ' to: ']);
      	[myDxl,myDxl_int,junk,crap,myDxl_stats] = regress(sdf(long_term_regress_begin(1):long_term_end(1),1),[timeinterval(long_term_regress_begin(1):long_term_end(1)) ones(long_term_end(1)-long_term_regress_begin(1)+1,1)],.05);
   		[myHxl,myHxl_int,junk,crap,myHxl_stats] = regress(A(long_term_regress_begin(1):long_term_end(1),1),[logtime(long_term_regress_begin(1):long_term_end(1)) ones(long_term_end(1)-long_term_regress_begin(1)+1,1)],.05);
   	...
   	if myDyl_stats(1,1) < good    % .7225 Corresponds to r=.85
      	a= sin(100*(.01:.01:300));
	%      sound(a);
      	disp(['Check end of long term region- r2 is ' num2str(myDyl_stats(1,1))]);
     		figure
      		plot(sdf(:,2))
      	...
      	long_term_end = input(['2000 does not work for Y. Long term region goes from ' num2str(short_term_end(2)) ' to: ']);
      	[myDyl,myDyl_int,junk,crap,myDyl_stats] = regress(sdf(long_term_regress_begin(2):long_term_end(2),2),[timeinterval(long_term_regress_begin(2):long_term_end(2)) ones(long_term_end(2)-long_term_regress_begin(2)+1,1)],.05);
   		[myHyl,myHyl_int,junk,crap,myHyl_stats] = regress(A(long_term_regress_begin(2):long_term_end(2),2),[logtime(long_term_regress_begin(2):long_term_end(2)) ones(long_term_end(2)-long_term_regress_begin(2)+1,1)],.05);
      ...
      if myDrl_stats(1,1) < good    % .7225 Corresponds to r=.85
      	a= sin(100*(.01:.01:300));
	%      sound(a);
      	disp(['Check end of long term region- r2 is ' num2str(myDrl_stats(1,1))]);
     		figure
      		plot(sdf(:,3))
      	...
      	long_term_end = input(['2000 does not work for R. Long term region goes from ' num2str(short_term_end(3)) ' to: ']);
      	[myDrl,myDrl_int,junk,crap,myDrl_stats] = regress(sdf(long_term_regress_begin(3):long_term_end(3),3),[timeinterval(long_term_regress_begin(3):long_term_end(3)) ones(long_term_end(3)-long_term_regress_begin(3)+1,1)],.05);
			[myHrl,myHrl_int,junk,crap,myHrl_stats] = regress(A(long_term_regress_begin(3):long_term_end(3),3),[logtime(long_term_regress_begin(3):long_term_end(3)) ones(long_term_end(3)-long_term_regress_begin(3)+1,1)],.05);
   		disp(['New r2 is ' num2str(myDyl_stats(1,1))]);
      end;
   ...
   %disp('Calculating Critical Points . . .');
